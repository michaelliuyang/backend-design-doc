...
This file is vendored from the upstream lanhu-mcp project and lightly adapted so
it can be imported by a local CLI script without requiring an MCP runtime.
"""
import asyncio
import os
import re
...
try:
    ...
    class Context:  # type: ignore[override]
        """Fallback context used when fastmcp is not installed."""

    class Image:  # type: ignore[override]
        """Fallback image wrapper used by the direct CLI serializer."""
    ...
    class FastMCP:  # type: ignore[override]
        ...
        def run(self, *_args, **_kwargs):
            raise RuntimeError("FastMCP is not available in direct CLI mode.")
...
def _require_playwright() -> None:
    if async_playwright is None:
        raise RuntimeError(
            "playwright is required for this command. Install project dependencies first."
        )
...
def _get_analysis_mode_options_by_role(user_role: str) -> str:
   ...
   - 前后端分工参考"""
    ...
    clarifier_option = """4️⃣ 【需求澄清】- 逐页分析 + 疑问汇总
   适合：产品经理自查、需求评审前准备、开发反馈需求问题
   输出内容：
   - 逐页功能理解与规则提取
   - 规则漏洞和疑问清单（按严重程度分级：🔴必须/🟡建议/🔵可选）
   - 功能介绍文档 + 待澄清文档"""

    backend_designer_option = """5️⃣ 【后端设计】- Service 分层 + 状态机 + DDL
   适合：后端开发、架构师、技术负责人
   前置条件：建议先用「需求澄清」模式产出澄清文档（可跳过）
   输出内容：
   - Service 分层设计（职责 + 接口 + 依赖关系）
   - 状态机设计（状态枚举 + 转换规则 + 流转图）
   - 核心业务流程图（含所有边界情况）
   - 完整 DDL（CREATE TABLE + 索引 + 注释）
   - DDL 与设计文档交叉校验 + 自动修正"""

    api_designer_option = """6️⃣ 【接口设计】- 逐页生成 API 接口定义
   适合：前后端联调、接口文档编写
   前置条件：必须提供 DDL 文件（接口参数使用 DDL 字段名）
   输出内容：
   - 逐页分析所需 API 接口（GET/POST）
   - POST Body 对象定义（对象名 + 字段列表）
   - 所有参数基于 DDL 字段（snake_case → camelCase）
   - 接口合并去重（避免接口爆炸）
   - 按页面维度拆分的最终接口文档"""
...
6. **⚡【必须】询问用户选择分析模式**（标记confirm_mode为in_progress）：
   ...
   ⚠️ 【后端设计模式特殊步骤】如果用户选择了「后端设计」(backend_designer)：
   在标记confirm_mode为completed后、执行步骤7之前，必须追加询问：
   ```
   您选择了「后端设计」模式。为了产出更准确的设计文档，请问：

   📋 是否有已完成的「需求澄清文档」？
   - 如果有：请粘贴澄清文档内容，或告诉我文档位置
   - 如果没有：我将基于原型页面直接进行后端设计（可能需要更多假设）

   请回复澄清文档内容，或回复「没有」跳过。
   ```
   等待用户回复后再继续步骤7。如果用户提供了澄清文档，在后续 STAGE 2/4 分析中必须将其作为设计依据。

   ⚠️ 【接口设计模式特殊步骤】如果用户选择了「接口设计」(api_designer)：
   在标记confirm_mode为completed后、执行步骤7之前，必须追加询问：
   ```
   您选择了「接口设计」模式。

   📋 请提供 DDL 文件（必须）：
   本模式需要 DDL 来定义接口参数名（使用 DDL 字段名转 camelCase），
   请粘贴 DDL 内容或告诉我 DDL 文件路径。

   ⚠️ 没有 DDL 无法继续，因为所有接口参数必须基于数据库字段定义。
   ```
   等待用户提供 DDL 后再继续步骤7。DDL 内容必须在后续 STAGE 2/4 中作为参数命名的唯一依据。
   同时，必须读取 references/api-design-rules.md 中的规则作为接口设计约定。
   如果用户坚持没有 DDL，拒绝继续此模式，建议用户先使用「后端设计」模式产出 DDL。
...
3. **根据分析模式输出不同内容**：
   ...
   【后端设计】逐页分析后端架构设计：
   - Service 层识别（归属 Service、职责、对外接口）
   - 状态机识别（状态枚举、转换规则、流转图）
   - 核心业务流程图（含所有分支、异常、边界情况）
   - 数据模型初步识别（实体、字段、关联）
   - 后端视角待澄清问题

   【接口设计】逐页分析所需 API 接口：
   - 页面操作 → API 接口映射
   - 参数定义（基于 DDL 字段名，camelCase）
   - POST Body 对象定义（对象名 + 字段）
   - 接口分类标签（查询/创建/修改/删除等）
   - 规则来源：references/api-design-rules.md
...
2. **汇总STAGE2所有结果，根据分析模式验证不同内容**：
   ...
   【需求澄清】验证：
   - 每个页面的功能是否都已理解？规则是否都已提取？
   - 疑问是否按严重程度正确分级？
   - 跨页面关联是否都已识别？

   【后端设计】验证：
   - 每个页面涉及的 Service 是否都已识别？
   - 状态机的状态转换是否完整（无遗漏的转换路径）？
   - 流程图是否覆盖了所有异常和边界情况？
   - 数据模型的字段是否和页面上的字段一一对应？

   【接口设计】验证：
   - 每个页面的用户操作是否都有对应接口？
   - 所有接口参数是否都来源于 DDL 字段？
   - 所有 POST 接口是否都定义了 Body 对象名和字段？
   - 所有接口是否都包含 userId 和 tenantId？
   - 是否存在可合并的冗余接口？
...
2. **根据分析模式生成对应交付物**（工具返回的 prompt 中有详细格式）：
   ...
   【后端设计】输出：后端设计文档 + DDL + 交叉校验（三段式）
   ```
   # 后端设计文档

   ## Part 1: 后端设计文档
   ## 📊 设计概览
   ## 🏗️ Service 分层设计（总览表 + 各 Service 详细设计）
   ## 🔄 状态机设计（状态枚举 + 转换表 + 流转图）
   ## 📈 核心业务流程图（含所有边界情况）
   ## ⚠️ 新一轮待澄清问题（后端视角）

   ## Part 2: DDL 设计
   ## 🗄️ 数据库设计（表结构总览 + 完整 CREATE TABLE + ER 关系图）

   ## Part 3: 交叉校验
   ## ✅ DDL 与设计文档匹配性校验（校验清单 + 修正记录 + 最终确认）
   ```

   【接口设计】输出：按页面维度的 API 接口文档（五段式）
   ```
   # API 接口设计文档

   ## Part 1: 接口合并去重（合并记录 + 统计）
   ## Part 2: 按页面维度的接口详情
     每个页面下列出所有接口（URL、方法、参数表、Body 对象、响应、规则）
   ## Part 3: 共享接口清单（跨页面复用）
   ## Part 4: DDL 与接口匹配性校验
   ## Part 5: 待确认问题（🔴阻塞/🟡建议/🔵优化）
   ```
...
3. **输出完成提示**（根据分析模式调整话术）：
   ...
   【需求澄清】
   "需求澄清报告已整理完毕，包含功能介绍和待澄清文档。"

   【后端设计】
   "后端设计文档已整理完毕，包含 Service 分层、状态机、流程图、DDL 及交叉校验结果。"

   【接口设计】
   "API 接口文档已整理完毕，按页面维度拆分，所有参数基于 DDL 字段定义。"
...
⚠️ TODO content字段规则（用户可见）：
  ...
  - 模式名 = 开发视角/测试视角/快速探索/需求澄清/后端设计/接口设计
...
⚠️ 分析模式必须由用户选择：
  ...
  - 用户可以说"开发"/"测试"/"快速探索"/"需求澄清"/"后端设计"/"接口设计"或自定义需求
...
def _get_stage2_prompt_clarifier() -> str:
    """获取需求澄清视角的 Stage 2 逐页分析 prompt"""
    return """
🧠 逐页深度分析（需求澄清视角）
...
**📖 页面功能理解**：
- 页面定位：[这个页面在整体业务中的角色和位置]
- 核心功能：[这个页面做什么，解决什么业务问题]
- 业务场景：[用户在什么场景下使用这个页面]
- 操作流程：[用户在这个页面上的主要操作路径]

**📋 规则提取**（逐条列出页面上写明的所有规则）：

### 业务规则
| 序号 | 规则描述 | 规则来源（引用原文） | 涉及字段/功能 |
|------|----------|---------------------|--------------|

### 校验规则
| 字段名 | 必填 | 格式/长度 | 校验规则 | 错误提示 |
|--------|------|-----------|----------|----------|
...
**⚠️ 疑问与规则漏洞**（逐条列出，标注严重程度）：

### 🔴 规则缺失（必须澄清，否则无法开发）
- ⚠️ [缺失什么]：[具体描述缺了什么规则，为什么必须补充]

### 🟡 逻辑矛盾/歧义（建议澄清，可能导致理解偏差）
- ⚠️ [矛盾/歧义点]：[具体描述，引用原文对比]

### 🔵 边界未定义（可后续补充，不阻塞主流程）
- 💡 [未定义的边界]：[具体描述，如极端情况、并发、数据量等]

**🔗 与其他页面的关联**：
- 数据依赖：依赖「XX页面」的[具体数据/状态]
- 数据输出：向「XX页面」提供[具体数据]
- 交互跳转：[与哪些页面有跳转关系]
- 潜在冲突：[与其他页面规则是否存在矛盾]
"""
...
def _get_stage2_prompt_backend_designer() -> str:
    """获取后端设计视角的 Stage 2 逐页分析 prompt"""
    return """
🧠 逐页深度分析（后端设计视角）
...
**🏗️ Service 层识别**：
- 本页涉及的业务领域：[领域名称]
- 建议归属 Service：[Service 名称]
- Service 职责描述：[一句话描述该 Service 的核心职责]
- 对外接口识别：
  | 接口名称 | 类型(查询/命令) | 输入参数 | 输出结果 | 业务规则 |
  |----------|----------------|----------|----------|----------|
...
- 状态转换规则：
  ...
  |----------|----------|----------|----------|--------|
- 状态流转图（Vertical Flow Diagram）

**📈 核心业务流程图**（含所有边界情况）：
- 流程名称：[核心业务操作名]
- 参与角色：[哪些角色参与]
- 流程图（Vertical Flow Diagram，必须包含）：
  - 正常路径（主流程）
  - 所有分支条件和判断
  - 异常/错误处理路径
  - 并发/竞态场景（如有）
...
**📊 数据模型初步识别**：
| 实体名称 | 核心字段 | 字段类型 | 是否必填 | 业务含义 | 关联实体 |
|----------|----------|----------|----------|----------|----------|

**⚠️ 后端视角待澄清问题**：
- 🔴 [阻塞设计]：[具体问题，如：并发控制策略未定义]
- 🟡 [影响设计]：[具体问题，如：是否需要审批流]
- 🔵 [优化相关]：[具体问题，如：是否需要缓存策略]
"""


def _get_stage2_prompt_api_designer() -> str:
    """获取接口设计视角的 Stage 2 逐页分析 prompt。
    规则来源：references/api-design-rules.md"""
    return """
🧠 逐页接口分析（接口设计视角）

⚠️ 重要前置：本模式要求用户提供 DDL 文件。所有接口参数必须使用 DDL 中的字段名（snake_case → lowerCamelCase）。
⚠️ 规则参考：本模式的设计规则定义在 references/api-design-rules.md 中，必须严格遵守。
...
逐个操作/交互分析，为每个需要后端支持的操作定义接口：

### 接口 1：[接口名称]
- URL: [HTTP方法] /api/v1/[模块]/[资源]
- 分类: [查询/详情/创建/修改/删除/状态变更/批量操作]
- 触发场景：[页面上什么操作触发此接口]

**参数定义**（GET = Query Params，POST = Body 对象）：

若为 POST，Body 对象名：[动作][实体]Request（如 CreateOrderRequest）

| 参数名(camelCase) | 类型 | 必填 | 来源DDL字段(snake_case) | 来源DDL表 | 说明 |
|-------------------|------|------|------------------------|----------|------|
| userId | Long | 是 | user_id | — | 操作用户(必传) |
| tenantId | Long | 是 | tenant_id | — | 租户(必传) |
| ... | ... | ... | ... | ... | ... |

**响应定义**：
- 响应对象名：[实体]VO（如 OrderVO）
- 核心响应字段：
  | 字段名(camelCase) | 类型 | 来源DDL字段 | 说明 |
  |-------------------|------|-----------|------|
...
def _get_stage4_prompt_clarifier() -> str:
    """获取需求澄清视角的 Stage 4 交付物 prompt"""
    return """
【STAGE 4 输出要求 - 需求澄清视角】
...
3. ## 📋 功能介绍（按模块汇总）
   ...
   每个模块下补充一段话描述模块整体定位和业务价值。

4. ## ⚠️ 待澄清文档（按严重程度排序）

   ### 🔴 规则缺失（必须澄清，阻塞开发）
   | 序号 | 所属页面 | 问题描述 | 影响范围 | 建议补充内容 |
   |------|----------|----------|----------|-------------|

   ### 🟡 逻辑矛盾/歧义（建议澄清，可能导致返工）
   | 序号 | 所属页面 | 矛盾描述 | 相关原文引用 | 建议处理方式 |
   |------|----------|----------|-------------|-------------|

   ### 🔵 边界未定义（可后续补充，不阻塞主流程）
   | 序号 | 所属页面 | 未定义场景 | 可能的默认行为 |
   |------|----------|-----------|--------------|
...
质量标准：产品经理看完知道要补什么，开发看完知道哪些能动哪些要等
"""


def _get_stage4_prompt_backend_designer() -> str:
    """获取后端设计视角的 Stage 4 交付物 prompt（三段式：设计文档 + DDL + 交叉校验）"""
    return """
【STAGE 4 输出要求 - 后端设计视角（三段式输出）】

⚠️ 重要：如果用户在 confirm_mode 阶段提供了澄清文档，必须将澄清文档中的已确认结论作为设计依据，
未澄清的问题标注为「待确认，当前按XX方案设计」。
...
2. ## 🏗️ Service 分层设计
   ...
   ### 总览
   | Service 名称 | 职责描述 | 依赖 Service | 对外接口数 |
   |-------------|----------|-------------|-----------|

   ### 各 Service 详细设计
   每个 Service 包含：
   - 职责边界：[明确该 Service 做什么、不做什么]
   - 对外接口列表：
     | 接口名称 | 类型 | 输入 | 输出 | 核心业务规则 |
   - 内部方法（关键的）：
     | 方法名 | 职责 | 调用的下游 |
   - 依赖关系说明
...
3. ## 🔄 状态机设计
   ...
   - 状态枚举表：
     | 状态值 | 状态名称 | 说明 | 是否终态 |
   - 状态转换表：
     | 当前状态 | 触发事件 | 目标状态 | 前置条件 | 副作用 | 失败处理 |
   - 状态流转图（Vertical Flow Diagram）

4. ## 📈 核心业务流程图（含所有边界情况）
   每个核心流程：
   - 流程名称 + 触发场景
   - Vertical Flow Diagram（必须包含）：
     ```
     [开始] → [权限校验] → [参数校验] → [业务规则校验]
       ↓ 失败 → [返回错误]
     [核心操作] → [状态变更] → [副作用处理]
...
6. ## 🗄️ 数据库设计
   ...
   ### 表结构总览
   | 表名 | 中文名 | 用途 | 核心字段数 | 关联表 | 对应 Service |
   |------|--------|------|-----------|--------|-------------|

   ### 完整 DDL
   每张表输出完整 CREATE TABLE 语句，要求：
   - 字段必须有 COMMENT
   - 包含合理的索引（主键、唯一索引、普通索引）
   - 包含外键关系说明（注释形式，不强制外键约束）
   - 状态字段必须注释枚举值含义
   - 包含 created_at / updated_at 等审计字段
   - 使用 utf8mb4 字符集

   ### ER 关系图（文字版）
   ```
   [表A] 1──N [表B]（通过 xxx_id 关联）
   [表B] N──M [表C]（通过中间表 table_b_c）
   ```
...
7. ## ✅ DDL 与设计文档匹配性校验
   ...
   | 校验维度 | 检查内容 | 结果(✅/❌) | 问题描述 |
   |----------|----------|-----------|----------|
   | 字段覆盖 | Service 接口引用的字段是否都在 DDL 中 | | |
   | 状态一致 | 状态机枚举值是否和表字段枚举一致 | | |
   | 流程覆盖 | 流程图中的数据操作是否有对应表和字段 | | |
   | 关联一致 | 表关联关系是否和 Service 依赖一致 | | |
   | 索引合理 | 查询接口的条件字段是否有索引 | | |
   | 约束完整 | 业务规则中的唯一性约束是否体现在 DDL | | |

   ### 🔧 修正记录
   如有不一致，逐条记录并直接修正：
   | 序号 | 不一致描述 | 修正内容 | 修正位置(设计文档/DDL) |
   |------|-----------|----------|---------------------|

   ⚠️ 修正后，将修正内容回写到 Part 1 或 Part 2 的对应位置，确保最终输出是一致的。

   ### ✅ 最终确认
   - [ ] 所有 Service 接口的字段在 DDL 中都有定义
   - [ ] 所有状态机的状态值在 DDL 中都有对应枚举
   - [ ] 所有核心流程的数据操作都有表支撑
   - [ ] DDL 的索引覆盖了主要查询场景

质量标准：后端开发拿到文档可以直接编码，DBA 拿到 DDL 可以直接建表
"""


def _get_stage4_prompt_api_designer() -> str:
    """获取接口设计视角的 Stage 4 交付物 prompt（按页面维度输出 + 接口合并 + 校验）"""
    return """
【STAGE 4 输出要求 - 接口设计视角】

⚠️ 重要：
- 所有接口参数必须使用 DDL 字段名（snake_case → lowerCamelCase），禁止自造字段名。
- 规则参考：references/api-design-rules.md 中的所有规则必须逐条遵守。
- 本阶段需要执行接口合并去重，避免接口爆炸。
...
1. ## 🔄 接口合并分析
   ...
   将 STAGE 2 各页面的接口进行合并去重：

   ### 合并记录
   | 合并前（页面A接口 + 页面B接口） | 合并后接口 | 合并原因 |
   |-------------------------------|-----------|----------|
...
3. ## 📄 按页面拆分接口详情
   ...
   #### 接口 1：[接口名称]
   - URL: [GET/POST] /api/v1/[模块]/[资源]
   - 分类: [查询/详情/创建/修改/删除/状态变更/批量操作]
   - 触发场景：[页面上哪个操作]

   **请求参数**（GET = Query Params / POST = Body）：

   POST Body 对象名：`[动作][实体]Request`

   | 参数名 | 类型 | 必填 | 来源DDL表.字段 | 说明 |
   |--------|------|------|---------------|------|
   | userId | Long | 是 | — | 操作用户(必传) |
   | tenantId | Long | 是 | — | 租户(必传) |
   | ... | ... | ... | ... | ... |
...
5. ## ✅ DDL 字段覆盖校验
   ...
   | 所有接口参数都有对应 DDL 字段 | | |
   | 所有 camelCase 命名正确 | | |
   | 所有 POST Body 对象已定义名称和字段 | | |
   | 所有接口包含 userId + tenantId | | |
   | 无自造字段（所有字段可溯源到 DDL） | | |
...
6. ## ⚠️ 接口设计待确认
   ### 🔴 阻塞（DDL 中缺少字段，无法定义接口参数）
   | 序号 | 涉及接口 | 缺失字段 | 需要的 DDL 表 |
   ### 🟡 建议确认（接口粒度、合并策略）
   | 序号 | 问题描述 | 影响接口 |
   ### 🔵 优化建议（分页策略、缓存、批量限制）
   | 序号 | 建议内容 | 涉及接口 |

质量标准：前端拿到文档可以直接对接，后端拿到文档可以直接写 Controller/Service，
所有参数名与 DDL 一一对应无歧义
"""


def _get_rd_review_score_rubric_prompt() -> str:
    """统一研发评审评分 rubric。"""
    return """
【统一研发评审评分要求】

在完成当前模式主体分析后，必须基于 references/rd-review-score-rubric.md 输出统一评分表。
评分表必须使用固定字段、固定顺序、固定等级，不得因分析模式不同而改变结构。
...
4. 每个维度必须给出：score / grade / verdict / strengths / issues / missing_evidence / impact / recommendations
5. 再输出：
   - `## 🚫 阻塞项`
   - `## ⚠️ 待确认项`
   - `## 📌 主要扣分证据`
...
评分原则：
- 必须基于当前页面/设计/规则/接口证据打分
- 必须区分事实与推断
- 发现阻塞级问题时，总评不得高于 C，严重时直接判定阻塞
- 不得只给总分，必须解释扣分证据和对研发的影响
"""


def _append_rd_review_scorecard(stage_prompt: str) -> str:
    """为各分析模式附加统一评分表要求。"""
    return stage_prompt.rstrip() + "\n\n" + _get_rd_review_score_rubric_prompt()


def _get_design_analysis_mode_prompt(analysis_mode: str) -> dict:
    """设计稿分析沿用统一的分析模式与评分输出。"""
    mode_prompts = _get_analysis_mode_prompt(analysis_mode)
    return {
        "mode_name": mode_prompts["mode_name"],
        "mode_desc": mode_prompts["mode_desc"],
        "stage2_prompt": _append_rd_review_scorecard(mode_prompts["stage2_prompt"]),
        "stage4_prompt": _append_rd_review_scorecard(mode_prompts["stage4_prompt"]),
    }
...
def _get_analysis_mode_prompt(analysis_mode: str) -> dict:
    ...
    Args:
        analysis_mode: 分析模式 (developer/tester/explorer/clarifier/backend_designer/api_designer)
    ...
    if analysis_mode == "tester":
        return {
            "mode_name": "测试视角",
            "mode_desc": "提取测试场景、校验规则、异常清单，并输出统一研发评审评分表",
            "stage2_prompt": _append_rd_review_scorecard(_get_stage2_prompt_tester()),
            "stage4_prompt": _append_rd_review_scorecard(_get_stage4_prompt_tester())
        }
    elif analysis_mode == "explorer":
        return {
            "mode_name": "快速探索",
            "mode_desc": "提取核心功能、依赖关系、评审要点，并输出统一研发评审评分表",
            "stage2_prompt": _append_rd_review_scorecard(_get_stage2_prompt_explorer()),
            "stage4_prompt": _append_rd_review_scorecard(_get_stage4_prompt_explorer())
        }
    elif analysis_mode == "clarifier":
        return {
            "mode_name": "需求澄清",
            "mode_desc": "逐页分析功能与规则，汇总疑问和规则漏洞，并输出统一研发评审评分表",
            "stage2_prompt": _append_rd_review_scorecard(_get_stage2_prompt_clarifier()),
            "stage4_prompt": _append_rd_review_scorecard(_get_stage4_prompt_clarifier())
        }
    elif analysis_mode == "backend_designer":
        return {
            "mode_name": "后端设计",
            "mode_desc": "Service 分层 + 状态机 + 流程图 + DDL + 交叉校验，并输出统一研发评审评分表",
            "stage2_prompt": _append_rd_review_scorecard(_get_stage2_prompt_backend_designer()),
            "stage4_prompt": _append_rd_review_scorecard(_get_stage4_prompt_backend_designer())
        }
    elif analysis_mode == "api_designer":
        return {
            "mode_name": "接口设计",
            "mode_desc": "逐页生成 API 接口，基于 DDL 字段定义参数，合并去重，并输出统一研发评审评分表",
            "stage2_prompt": _append_rd_review_scorecard(_get_stage2_prompt_api_designer()),
            "stage4_prompt": _append_rd_review_scorecard(_get_stage4_prompt_api_designer())
        }
    else:  # developer (default)
        return {
            "mode_name": "开发视角",
            "mode_desc": "提取所有细节、字段规则、完整流程，并输出统一研发评审评分表",
            "stage2_prompt": _append_rd_review_scorecard(_get_stage2_prompt_developer()),
            "stage4_prompt": _append_rd_review_scorecard(_get_stage4_prompt_developer())
        }
...
async def lanhu_get_ai_analyze_page_result(
        ...
        analysis_mode: Annotated[str, "Analysis perspective (MUST be chosen by user after STAGE 1): 'developer' (detailed for coding), 'tester' (test scenarios/validation), 'explorer' (quick overview for review), 'clarifier' (page-by-page analysis + clarification doc), 'backend_designer' (service design + state machine + DDL), 'api_designer' (API interface design per page, requires DDL). Default: 'developer'"] = "developer",
        ctx: Context = None
    ...
    2. STAGE 2: Call with mode="full" for each group (output format varies by analysis_mode)
       ...
       - clarifier: Page-by-page deep analysis (function understanding, rule extraction, questions/gaps)
       - backend_designer: Per-page backend design (service layer, state machines, flow diagrams, data models)
       - api_designer: Per-page API interface design (GET/POST endpoints, params from DDL fields)
    ...
    4. STAGE 4: Generate deliverable (format varies by analysis_mode)
       ...
       - clarifier: Feature introduction + clarification doc (questions sorted by severity)
       - backend_designer: Backend design doc (service + state machine + flow) + DDL (CREATE TABLE) + cross-check
       - api_designer: API doc by page (endpoints + params + body objects) + merge dedup + DDL cross-check
...
async def lanhu_get_ai_analyze_design_result(
        ...
        analysis_mode: Annotated[str, "Analysis perspective for design review: 'developer', 'tester', 'explorer', 'clarifier', 'backend_designer', 'api_designer'. Default: 'developer'"] = "developer",
        ctx: Context = None
) -> List[Union[str, Image]]:
    """
    ...
    The final analysis must append the unified RD review scorecard defined in
    references/rd-review-score-rubric.md, using the same fields and grading rules as PRD analysis.
    ...
    try:
        ...
        summary_text += f"🤖 Analysis Mode: {analysis_mode}\n"
        ...
        design_mode_prompts = _get_design_analysis_mode_prompt(analysis_mode)
        summary_text += "=" * 60 + "\n"
        summary_text += f"🤖 设计稿分析模式：【{design_mode_prompts['mode_name']}】\n"
        summary_text += f"📋 {design_mode_prompts['mode_desc']}\n"
        summary_text += "=" * 60 + "\n"
        summary_text += "分析完设计稿后，必须在主体分析结果后追加统一研发评审评分表。\n"
        summary_text += design_mode_prompts['stage2_prompt']
        summary_text += "\n" + "=" * 60 + "\n"
        summary_text += "📝 最终交付时，仍需遵守以下 STAGE 4 输出要求：\n"
        summary_text += design_mode_prompts['stage4_prompt']
        summary_text += "\n" + "=" * 60 + "\n\n"
...
if __name__ == "__main__":
    raise SystemExit(
        "This vendored module is meant to be called from lanhu_cli.py, not run as an MCP server."
    )
