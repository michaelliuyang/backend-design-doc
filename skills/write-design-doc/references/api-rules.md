...
**所有接口**都必须说明当前用户与租户上下文的来源。优先沿用现有项目的鉴权上下文、登录态、网关注入参数或线程上下文；不要为了满足文档模板而强行把上下文字段暴露给前端。

当现有项目明确要求前端传参时，才将以下字段放入 Query Params 或 Request Body：
...
接口设计必须在每个接口中写明：
- 用户上下文来源：Token / Session / 网关注入 / 请求参数 / 待确认。
- 租户上下文来源：Token / Session / 网关注入 / 请求参数 / 待确认。
- 权限校验点：角色、权限码、资源归属或待确认。

若缺少上下文规则，不要自造确定结论；在 `待确认事项` 中列出影响和建议默认值。
